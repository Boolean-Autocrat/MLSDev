...
var treeConnector = '/include/class/ckeditor/custom/filemanager/jqueryFileTree.php';
...
var selectItem = function (data) {
	if (baseUrl !== '' && basePath != '') {
		var url = smartPath(baseUrl, data['Path'].replace(basePath, ""));
	} else {
	...
	if (window.opener) {
		if ($.urlParam('CKEditor')) {
			...
			if (data['Properties']['Width'] != '') {
				...
				window.opener.SetUrl(data['Preview']);
			}
		}
...
var addNode = function (path, name) {
	...
	if (!parentNode.find('ul').length) parentNode.append('<ul></ul>');
		...
		if (data) {
			if ($('#fileinfo').data('view') == 'grid') {
				...
				for (const [key, value] of Object.entries(data)) {
					...
					result += '<li><div class="clip"><img src="' + value['Preview'] + '" width="' + scaledWidth + '" alt="' + value['Path'] + '" /></div><p>' + value['Filename'] + '</p>';
				...
				for (const [key, value] of Object.entries(data)) {
					...
					var props = value['Properties'];
					result += '<tr>';
					result += '<td title="' + path + '">' + value['Filename'] + '</td>';
