 ...
 */
//$root = null;
$root = $_SESSION['filemanager_basepath'].$_SESSION['filemanager_pathpart'];
if (!$root) {
    exit("ERROR: Root filesystem directory not set in jqueryFileTree.php");
}
// phpcs:ignore
$postDir = ((isset($_POST['dir']) && realpath($_POST['dir']) !== false && realpath($_POST['dir']) !== '/') ? realpath($_POST['dir']): $root);
if (strpos($postDir, $root) !== 0) {
    exit("ERROR: Root filesystem directory cannot be traversed");
}
...
if (file_exists($postDir)) {
    ...
    if (count($files) > 2) { // The 2 accounts for . and ..
        ...
        foreach ($files as $file) {
            $htmlRel	= htmlentities($postDir . $file, ENT_QUOTES);
